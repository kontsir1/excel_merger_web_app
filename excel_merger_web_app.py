...
def main():
    ...
    if df1 is not None:
        ...
        common_column1 = st.selectbox("Select the common column in the first file", df1.columns)
        common_column2 = st.selectbox("Select the common column in the second file", df2.columns)
        df_merged = merge_dataframes(df1, df2, common_column1, common_column2)
        
        selected_columns = st.multiselect("Select the columns to export", df_merged.columns)
        if st.button("Export"):
            ...
            if file_type == "CSV":
                ...
                xlsx_file = BytesIO()
                df_merged[selected_columns].to_excel(xlsx_file, index=False)
                xlsx_file.seek(0)
                xlsx_b64 = base64.b64encode(xlsx_file.read()).decode()
                xlsx_href = f'<a href="data:file/xlsx;base64,{xlsx_b64}" download="merged_data.xlsx">Download XLSX</a>'
                st.sidebar.markdown(xlsx_href, unsafe_allow_html=True)
